...
def get_candidates():
    ...
    d1_opts = ["dD1ABI", "D1ABI", "dDlABI", "DlABI"]
    lw_opts = ["Lw1Ue", "LwlUe"]
    z07_opts = ["z07A1", "z07Al"]
    hp_opts = ["HP1An", "HPlAn"]
    gy_opts = ["GYI1", "GYIl"]
    mup_opts = ["MUP1", "MUPl"]
    iz_opts = ["Iz9r", "iz9r", "1z9r"]
                                        ...
                                        for gy in gy_opts:
                                            for mup in mup_opts:
                                                for iz in iz_opts:
                                                    ...
                                                    p3 = f"z0RA{w}6nz{q7}HGHgf{d1}-T42u3axtYzrb{lw}PHOoTJoMhfS_XejvziFv48qZhBt9p40q0Uzy6g_qd3Dh2IUWPZdWSPNuoGRVMsrZ-aqN3N9cqwwDDw"
                                                    p4 = f"{z07}TBQ3R0Ykhrfp0JnTMMUGxstP9qZ4R5re{hp}LE5sMfPMZmZTUREkyG7Sy_CGmHhoM5s7AFgJWoge6{gy}yXEOCayt7xbH7neL5UBAtbi8T4yEhfEXB"
                                                    p5 = f"NrhWLL9MkmMUP{mup[3]}UMFAf-cUBQoXthi{iz}UbUu7xw94bc"
...
async def main():
    ...
    sem = asyncio.Semaphore(50)  # Correctly initialized inside the running event loop
    
    # We will run them in tasks
    tasks = []
    for idx, cand in enumerate(candidates):
        tasks.append(test_candidate(sem, cand, idx, total))
        
    print("Starting async tests. This might take a few minutes...")
